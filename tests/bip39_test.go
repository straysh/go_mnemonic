...
func TestBip39(t *testing.T) {
	...
	//vector := testVectors()["english"][0]
	//entropy, err := hex.DecodeString(vector.entropy)
	//assert.NoError(t, err)
	//
	//m, err := mnemonic.FromEntropy(entropy, password)
	//assert.NoError(t, err)
	//assert.Equal(t, vector.mnemonic, m.Mnemonic())
	//assert.Equal(t, vector.seed, m.GetSeedHex())
	...
	for _, vector := range testVectors()["english"] {
		...
		_, err = m.FromEntropy(entropy, password)
		assert.NoError(t, err)
		assert.Equal(t, vector.mnemonic, m.Mnemonic())
		assert.Equal(t, vector.seed, utils.BufferToHexa(m.Seed()))
	}
}

func testVectors() map[string][]Vector {
